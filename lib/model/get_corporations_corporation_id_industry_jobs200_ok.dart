...
class GetCorporationsCorporationIdIndustryJobs200Ok {
  ...
  @override
  String toString() {
    return 'GetCorporationsCorporationIdIndustryJobs200Ok[activityId=$activityId, blueprintId=$blueprintId, blueprintLocationId=$blueprintLocationId, blueprintTypeId=$blueprintTypeId, completedCharacterId=$completedCharacterId, completedDate=$completedDate, cost=$cost, duration=$duration, endDate=$endDate, facilityId=$facilityId, installerId=$installerId, jobId=$jobId, licensedRuns=$licensedRuns, locationId=$locationId, outputLocationId=$outputLocationId, pauseDate=$pauseDate, probability=$probability, productTypeId=$productTypeId, runs=$runs, startDate=$startDate, status=$status, successfulRuns=$successfulRuns, ]';
  }

  GetCorporationsCorporationIdIndustryJobs200Ok.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<GetCorporationsCorporationIdIndustryJobs200Ok> listFromJson(List<dynamic> json) {
    return json == null ? List<GetCorporationsCorporationIdIndustryJobs200Ok>() : json.map((value) => GetCorporationsCorporationIdIndustryJobs200Ok.fromJson(value)).toList();
  }

  static Map<String, GetCorporationsCorporationIdIndustryJobs200Ok> mapFromJson(Map<String, dynamic> json) {
    var map = Map<String, GetCorporationsCorporationIdIndustryJobs200Ok>();
    if (json != null && json.isNotEmpty) {
      json.forEach((String key, dynamic value) => map[key] = GetCorporationsCorporationIdIndustryJobs200Ok.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of GetCorporationsCorporationIdIndustryJobs200Ok-objects as value to a dart map
  static Map<String, List<GetCorporationsCorporationIdIndustryJobs200Ok>> mapListFromJson(Map<String, dynamic> json) {
    var map = Map<String, List<GetCorporationsCorporationIdIndustryJobs200Ok>>();
     if (json != null && json.isNotEmpty) {
       json.forEach((String key, dynamic value) {
         map[key] = GetCorporationsCorporationIdIndustryJobs200Ok.listFromJson(value);
       });
     }
     return map;
  }
